/* ============================================================================
   ...
   Mood background:
     - canvas color (black in dark mode, white in light mode)
     - three layered radial blobs that drift — tight in light mode
     - optional edge vignette (dark mode only)
     - velocity-driven brightness lift + film grain
   ========================================================================= */
...
void main() {
  ...
  float baseRadius = 0.92 * uGlowScale * (1.0 + sin(waveTime * 0.55) * 0.04);
  ...
  float radius1 = uBlobRadius * uGlowScale * (1.0 + sin(waveTime * 0.72) * 0.045);
  ...
  float radius2 = uBlobRadiusSecondary * uGlowScale * (1.0 + cos(waveTime * 0.58 + 1.2) * 0.04);
  ...
  float radius3 = 0.28 * uGlowScale;
  float blob3 = smoothstep(radius3, 0.0, distance(uv, blob3Center));
  vec3 blob3Soft = mix(uBlob2Color, uCanvasColor, 0.42);
